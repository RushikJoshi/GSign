...
const FieldEditorStep = ({ onBack, data, onSend, isSending }) => {
    ...
    <div className="fixed inset-0 bg-slate-100 z-[100] flex flex-col overflow-hidden text-slate-800 font-sans">
      ...
      <header className="h-[56px] bg-white border-b border-slate-200 flex items-center justify-between px-4 shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={onBack} disabled={isSending} className="text-[14px] font-bold text-slate-600 px-4 hover:bg-slate-50 h-8 rounded border border-slate-200 bg-white disabled:opacity-50 transition">Back</button>
          <button
            onClick={() => onSend({ fields: placedFields })}
            disabled={isSending}
            className="bg-[#249272] text-white px-6 h-8 rounded text-[14px] font-bold flex items-center gap-2 hover:bg-[#1e7a5f] transition shadow-sm disabled:opacity-75"
          >
            {isSending ? "Sending..." : "Send"} <Send className="w-3.5 h-3.5" />
          </button>
        </div>
      </header>
